...
| **Docker (full)** | Qdrant server + Redis + Postgres + optional `bge-m3` | `bash setup-docker.sh` (~3 min) | ~3 GB | Học viên muốn trải nghiệm production stack thật, có Docker Desktop |
...
Yêu cầu: Docker Desktop ≥ 4.x, RAM ≥ 8 GB free, port 6333/6379 không xung đột.
Endpoints: Qdrant http://localhost:6333 · Redis :6379 · Postgres :15432

Embedding backend mặc định vẫn là `fastembed` để chạy ổn trên nhiều máy; nếu muốn
đổi sang `bge-m3`, chỉ cần set `EMBEDDING_BACKEND=bge-m3` trong `.env`.

Nếu máy bạn đã có PostgreSQL chạy trên 5432, repo này tự chuyển Docker Postgres sang
`15432` qua `POSTGRES_HOST_PORT` để tránh conflict.
...
| Docker path: Postgres 5432 đã bị chiếm | Giữ `POSTGRES_HOST_PORT=15432` trong `.env` |
